...
# --- GANZE APP IN EINEM TRY-BLOCK (damit niemals weißer Bildschirm) ---
try:
    st.markdown("""
        <style>
        ...
        try:
            ...
            for symbol in assets:
                try:
                    ...
                    # Wenn ein Asset fehlschlägt, füge trotzdem eine Zeile mit Fehlerinfo ein
                    results.append({"Asset": symbol, "Kurs": "Fehler", "5m_Sig": "N/A", "15m_Sig": "N/A", "1h_Sig": "N/A", "4h_Sig": "N/A", "1d_Sig": "N/A"})
        except Exception as e:
            st.error(f"❌ Kritischer Fehler beim Abrufen der Marktdaten: {e}")
            return pd.DataFrame()
    ...
    # --- TABELLE ANZEIGEN (OHNE STYLING, ABER ROBUST) ---
    if df_market.empty:
        st.warning("⚠️ Keine Daten von Kraken erhalten. Prüfe deine Internetverbindung oder die Asset-Liste.")
    else:
        # Sicherstellen, dass alle Signal-Spalten existieren (füge fehlende mit 'N/A' hinzu)
        for tf in ['5m', '15m', '1h', '4h', '1d']:
            col = f"{tf}_Sig"
            if col not in df_market.columns:
                df_market[col] = "N/A"
        # Tabelle anzeigen (ohne aufwändiges Styling)
        st.dataframe(df_market, use_container_width=True, hide_index=True, height=600)

    st.markdown("---")

    # --- UNTERER BEREICH (GEDANKEN, HANDELSPLATZ, CHAT) ---
    ...
    # FALLBACK: Wenn GAR NICHTS läuft, zeige den Fehler in einer einfachen Nachricht
    st.error(f"🚨 Schwerer Fehler im Dashboard:\n\n{e}")
    st.info("Bitte überprüfe die Render-Logs für mehr Details.")    return 100 - (100 / (1 + rs))
