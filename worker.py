...
# --- DATEN FÜR EIN ASSET HOLEN (KRYPTO + AKTIEN) ---
...
def get_entry_decision(market_data):
    ...
    prompt = f"""
    Du bist ein aggressiver Scalper. Marktdaten für {market_data['symbol']}:
    ...
    """
    answer, _ = router.route(prompt, system_context="Antworte NUR mit JSON.", preferred_model="groq")
...
def analyze_learn(asset, entry_price, exit_price, pnl, reasoning):
    ...
    prompt = f"""
    Trade auf {asset} abgeschlossen mit {profit_text} von ${pnl:.2f}.
    Einstieg: {entry_price}, Ausstieg: {exit_price}. Begründung: {reasoning}.
    Lehre mich eine Lektion daraus.
    """
    router = ModelRouter()
...
# --- HAUPTLOOP ---
def main_loop():
    print("🧠 KI-Scalper gestartet. 24/7 aktiv.", flush=True)
